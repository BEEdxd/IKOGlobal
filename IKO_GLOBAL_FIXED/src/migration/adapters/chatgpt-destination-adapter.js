...
class ChatGPTDestinationAdapter extends DestinationAdapter {
    constructor() { super("ChatGPT"); }
    ...
    async openDestination(migrationPackage) {
        ...
        const response = await new Promise((resolve, reject) => {
            try {
                chrome.runtime.sendMessage(
                    { type: "IKO_OPEN_MIGRATION_DESTINATION", destination: "chatgpt" },
                    result => {
                        const error = chrome.runtime.lastError;
                        if (error) reject(new AdapterError(error.message));
                        else resolve(result);
        ...
        if (!response?.success) {
            throw new AdapterError(response?.error || "Could not open ChatGPT.");
        }

        return { status: "opened", destination: "ChatGPT", url: response.url, tabId: response.tabId };
    }

    async prepareMigration(migrationPackage) {
        if (!migrationPackage) throw new AdapterError("Migration package is required.");
        return { status: "prepared", destination: "ChatGPT", package: migrationPackage };
    }
...
module.exports = { ChatGPTDestinationAdapter };
